const { arBNB, arCAKE, arPancakeswapBUSDBNB, arPancakeswapCAKEBNB,
    arSUSHI, arSOL, arOSMO, arQUICK, arCRV, arRAY, arDOT, arTRU, arOsmosisATOMOSMO, arQuickswapETHUSDC,
    arQuickswapETHQUICK, arRaydiumRAYSOL, arRaydiumRAYUSDT, arUniswapETHUSDT, arCurveDAIUSDCUSDT, arSushiswapETHTRU } = require ('../config/address.js');
...
async function feedPrices(state){
    try {
        ...
        const busdBNBLpPrice = state.bsc.pancakeswap.busdBnb.busdBnbLpTokenPrice.lpTokenPrice
        const cakeBNBLpPrice = state.bsc.pancakeswap.cakeBnb.cakeBnbLpTokenPrice.lpTokenPrice
        ...
        const uniswapETHUSDTPrice = state.eth.ethdata.ethUsdt.ethUsdtLpTokenPrice.lpTokenPrice
        const sushiswapETHTRUPrice = state.eth.ethdata.ethTru.truEthLpTokenPrice.lpTokenPrice
        const quickswapETHQUICKPrice = state.poly.polygonData.quickEth.quickEthLpTokenPrice
        const quickswapETHUSDCPrice = state.poly.polygonData.ethUsdc.ethUsdcLpTokenPrice
        const atomOsmoLpTokenPrice = state.osmosis.atomOsmoLpTokenPrice

        /**user readable price -- end **/
        /**Array of all address**/
        let tokensArray = [arBNB, arCAKE, arPancakeswapBUSDBNB, arPancakeswapCAKEBNB, arSUSHI, arSOL, arOSMO,
        ...
        let priceArray = [bnbPrice, cakePrice, busdBNBLpPrice, cakeBNBLpPrice, sushiPrice, solPrice, osmoPrice, quickPrice, 
            rayPrice, dotPrice, truPrice, crvPrice,
            quickswapETHUSDCPrice, quickswapETHQUICKPrice, raydiumRAYSOLPrice, raydiumRAYUSDTPrice, uniswapETHUSDTPrice, sushiswapETHTRUPrice,
