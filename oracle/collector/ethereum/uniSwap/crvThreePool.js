...
const {totalGauageAddress, gaugeAddress, priceFeedCrvAddress} = require('../../libs/address');
const {gauageThreePool_abi, totalGauage_abi, priceCrv_abi  } = require('../../libs/abis');
const percent =100
...
async function crv_three_pool(){
    try{
        ...
        const priceCrvContract = new web3.eth.Contract(priceCrv_abi, priceFeedCrvAddress)
        //live price of eth
        const crvPriceRoundData = await priceCrvContract.methods.latestRoundData().call()
        ...
        const crvPrice = await crvPriceRoundAnswer/Math.pow(10,crvPriceDecimals)
