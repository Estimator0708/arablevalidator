...
async function collect_osmosis() {
  ...
  let priceData = (
    await coinGeckoApi.simple.price({
      ids: ['cosmos', 'osmosis'],
      vs_currencies: ['usd'],
    })
  ).data;
  ...
  const poolTotalValueLocked =
    totalLpAtom * atomPrice + totalLpOsmo * osmoPrice;
  ...
  const epochProvision = await axios
    ...
    .then((res) => res.data.epoch_provisions);
  ...
  return {
    ...
    atomOsmoLpTokenReward14DaysBondedAprPct:
  ...
  function computeLpRewardsApr(duration) {
    ...
    const yearProvisionToPots =
      yearProvision * mintParams.distribution_proportions.pool_incentives;
